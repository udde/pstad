...
function init(gl) {
    ...
    for(var x = -blocks.x/2, k=0; x < blocks.x/2; x++, k++){
        ...
        console.log(k);
        console.log(size);
        console.log(i*blocks.x*blocks.y + j*blocks.y + k);
        meshBlocks[i*blocks.x*blocks.y + j*blocks.y + k] = generateMesh(gl,i+1, res, (x + 0.5)*size, (y + 0.5)*size );
        console.log(meshBlocks[i*blocks.x*blocks.y + j*blocks.y + k]);
        //i = 0 -> -res/(blocks/2) = -res/2 i båda
        //i = i  -res/2  & +res/2
